...
$(document).ready(function () {
  $('.sidebarTrigger').on('click touchstart', function (e) {
      ...
      var $sidebar = $('nav[data-sidebar-id="' + sidebarId + '"]');

      if ($sidebar.hasClass('active')) {
          hideSidebar(); // اگر منو باز است، ببند
      } else {
          // بستن منوهای قبلی
          $('nav.sidebar').removeClass('active visible animate');
          // نمایش منوی جدید
          $sidebar.addClass('visible active animate');
          window.sidebarShown = 1;
      }
  });

  // بستن منو وقتی روی خارج از آن کلیک شود
  $(document).on('click touchstart', function (e) {
      if (!$(e.target).closest('nav.sidebar, .sidebarTrigger').length && window.sidebarShown) {
          hideSidebar();
      }
  });
  ...
  window.hideSidebar = function () {
      $('nav.sidebar').removeClass('active animate');
      setTimeout(function () {
...
if (!jQuery.fn.removeClassByPrefix) {
